...
class GiteeAPIClient(BaseAPIClient):
    ...
    async def get_pr_labels_async(self, owner: str, repo: str, pr_id: int) -> Optional[List[Dict[str, Any]]]:
        url = f"{self.api_url}/repos/{owner}/{repo}/pulls/{pr_id}/labels"
    ...
    async def get_pr_details_async(self, owner: str, repo: str, pr_id: int) -> Optional[Dict[str, Any]]:
        url = f"{self.api_url}/repos/{owner}/{repo}/pulls/{pr_id}"
        result = await self._make_request("GET", url)
        if result is not None:
    ...
    async def get_author_prs_async(self, owner: str, repo: str, author: str) -> Optional[List[Dict[str, Any]]]:
        url = f"{self.api_url}/repos/{owner}/{repo}/pulls"
        params = {
            "state": "all",
            "sort": "updated",
    ...
    async def add_pr_labels_async(self, owner: str, repo: str, pr_id: int, labels: List[str]) -> Optional[List[Dict[str, Any]]]:
        url = f"{self.api_url}/repos/{owner}/{repo}/pulls/{pr_id}/labels"
        data = {"labels": labels}
